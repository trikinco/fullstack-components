...
export const Nav = ({ children, className, ...rest }: NavProps) => {
	...
	return (
		<>
			<div className="md:flex gap-3 justify-between text-sm text-red-700 dark:text-white p-6 bg-red-200 dark:bg-red-800 border-t border-red-500/10 dark:border-transparent">
				<p>
					<strong>
						<Link className="focus-ring" href={URL_GITHUB}>
							{NAME_LIB} is no longer maintained.
						</Link>
					</strong>
				</p>
				<p className="hidden md:block">Use at your own risk.</p>
			</div>
			<nav
				ref={ref}
				className={merge(
