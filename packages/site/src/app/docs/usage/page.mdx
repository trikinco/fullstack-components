...
Learn the basics of working with {NAME_LIB}.
...
For more information, see the [Next.js documentation](https://nextjs.org/docs/app/api-reference/file-conventions/metadata/sitemap).
...
Ensure your sitemap is accessible at `/sitemap.xml` in your application, and proceed to the next step.

### Step 2: Configure the API handler route

In the API handler route, add the following configuration to use the `useNotFoundEnhancement` hook shown later.
...
Congratulations on starting your journey with {NAME_SHORT}! You've taken the first steps towards seamlessly integrating AI capabilities into your Next.js applications.
